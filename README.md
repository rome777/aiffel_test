...
9개를 만들면서 **첫 렌더 시점에** 경로 탐색은 3개만 하는 이유는, 9개를 모두 탐색하면 §8 V1의 5초 예산을 깨기 때문입니다. 나머지 6개는 추정값만 붙은 채 목록에 먼저 나오고, 첫 렌더가 끝난 뒤 **백그라운드에서 순차로** 탐색되어 값이 채워집니다. 사용자가 목록에서 누르면 그 후보가 먼저 탐색됩니다.

즉 5초 예산 안의 경로 호출은 3회이고, **세션 전체로는 최대 9회**입니다. 조건(프리셋·고속도로·존)이 바뀌면 `courseSetId`가 달라지므로 진행 중이던 백그라운드 탐색은 즉시 중단됩니다.
...
| --- | --- | --- |
| 🚶 가볍게 | 약 15 km | 7 ~ 13 km |
| 🚗 보통 (기본값) | 약 40 km | 18 ~ 36 km |
| 🛣️ 길게 | 약 60 km | 27 ~ 53 km |
| ✏️ 직접 | 10 ~ 120 km (슬라이더 5 km 단위) | 같은 식으로 환산 |
...
| POI 품질 | 리뷰 30건 이상 · 평점 3.5 이상 | 목적지는 후보 밀도가 높아 경유지(15건 · 4.0)보다 리뷰 하한을 높게, 평점 하한을 낮게 잡음 |
...
graph LR
    ...
    J --> L["💬 설명 계층<br/>LLM (미구현)<br/>추천 이유 한국어 요약"]
...
| 설명 | LLM | 선정 이유를 한국어 문장으로 요약 | 없음 (경로를 바꾸지 못함) · **미구현** (§9) |
| 결정 | 사용자 | 추천 수용 또는 목적지 직접 지정 | 있음 (최상위) |

경로 선택을 LLM이 아닌 **결정론적 코드**에 맡기는 이유는 검증 기준 V2(`정체 구간 비율 < 8%`)를 합격·불합격으로 쓰기 위해서입니다. 같은 입력에 같은 결과가 나와야 측정과 회귀 테스트가 성립합니다. LLM은 이미 정해진 결과를 설명하는 역할만 맡습니다. 현재 구현은 이 층이 비어 있고, 선정 이유를 규칙 기반 템플릿 문장으로 대신 적습니다(§9).
...
사용자가 실제로 지난 구간의 소요 시간을 누적해 `보정 계수 = 실측 속도 ÷ 예측 속도`를 만들고 L1 · L2 결과에 곱합니다. **표본 30건 미만 구간은 보정하지 않습니다**(§10). 현재 구현은 표본을 **읽기만 하고 쌓는 경로가 없어**, 화면에는 항상 `L3 보정 없음 · 표본 0/30`으로 표시됩니다(§9). 주말에만 쓰는 개인 기록으로는 표본이 더디게 쌓이므로, L3은 단독 소스가 아니라 위 두 단계를 덧칠하는 층입니다.

세 단계가 모두 실패하면 귀로 정체율을 **표시하지 않고** `귀가 시 재조회`를 안내합니다. 근거 없는 숫자를 채워 넣는 것보다 빈칸이 낫습니다. 다만 L2는 인라인 테이블이라 실패하지 않으므로, 실제로는 이 빈칸 상태에 도달하지 않습니다.

귀로 정체율은 단일 값이 아니라 **±4%p 범위**로 적습니다 — `귀로 예상 정체 6~14% (예측)`.
...
| 경유지 추천 | 선택 | 기본값 **`켜기`** — 첫 렌더 뒤 비동기 실행 (§6) |
| 식당 · 카페 취향 | 선택 | 기본값 `한식 위주` · `조망 카페` (§6.2) |
| 실시간 정체율 | 자동 수집 | 지도 API의 실시간 교통 레이어 |
| 고속/시내 비율 | 자동 산출 | 경로 구간별 도로 등급 합산 |
| 유종 | 선택 | 기본값 `휘발유` · 단가는 `/api/fuel` 로 갱신 (§5.3) |
| 출발 존 | 선택 | 기본값 `Z1 서울 중심부` · 위치를 못 받았을 때만 사용 (§2) |
| 실연비 이력 | 선택 | 차종 공인연비(고속 15.0 / 복합 12.0)로 대체. 입력 UI는 아직 없음 (§9) |
...
차량 제원은 공인연비 기본값(**고속 15.0 / 복합 12.0 km/L**)을 씁니다. 차종 등록과 실연비 이력 입력은 아직 없으므로, V3(±1.0 km/L)의 보정 루프는 설계만 있고 동작하지 않습니다(§9).

### 5.3 유가와 예상 기름값

연비만으로는 "이 코스가 얼마짜리인지" 감이 오지 않으므로 **왕복 기름값**을 함께 냅니다.

```
예상 주유량 = 왕복 총 거리 ÷ 예상 연비
예상 기름값 = 예상 주유량 × 유종 단가
```

| 유종 | 초기 단가 | 갱신 |
| --- | --- | --- |
| 휘발유 (기본값) | 1,861 원/L | 실시간 조회 성공 시 덮어씀 |
| 고급휘발유 | 2,320 원/L | 〃 |
| 경유 | 1,845 원/L | 〃 |

단가는 로컬 서버의 `/api/fuel` 엔드포인트에서 **전국 주유소 평균가**를 받아 갱신합니다(§9). 조회가 실패하거나 정적 서버로 열었으면 위 초기 단가를 그대로 씁니다 — 값을 못 받았다고 기름값 칸을 비우지는 않습니다.

선택한 유종은 `localStorage`(`sd.fuelPrice`)에 남아 다음 방문에도 유지되고, 유종을 바꾸면 경로 재탐색 없이 화면 숫자만 다시 그립니다.

---

## 6. 경유지 추천 — 한식 식당 · 카페 (선택)

기본은 **켜짐**입니다. 갈 때 · 올 때 경로 주변에서 **한식 위주 식당**과 **분위기 좋은 카페**를 찾아 경유지로 얹고, 토글로 끌 수 있습니다.

정체 회피가 이 에이전트의 1순위 요구(§3.1의 `w1 = 0.5`)이므로, 경유지는 **경로를 크게 흔들지 않는 범위에서만** 추가합니다. 맛집을 위해 정체를 감수하는 것은 사용자가 원한 것이 아닙니다. 기본으로 켜 두되 **첫 렌더가 끝난 뒤 비동기로** 조회하는 이유도 같습니다 — 무입력 진입 시 5초 안에 코스를 그리는 것(§8 V1')이 이 페이지의 정체성이고, POI 조회는 그 경로에 끼어들면 안 됩니다.

```mermaid
graph TD
    A["경로 확정 (§3)<br/>구간별 통과 예정 시각 보유"] --> B{"경유지 추천<br/>켜짐?"}
    B -- "꺼짐" --> Z["경유지 없이 렌더링"]
    B -- "켜짐 (기본)" --> C["① 통과 시각 → 슬롯 배정<br/>식사 · 카페"]
    C --> D["② 경로 버퍼 내 POI 조회<br/>반경 2 km · 영업 중"]
    D --> E["③ 규칙 필터<br/>한식 분류 · 평점 · 리뷰 수"]
    E --> F["④ 우회 비용 재계산<br/>추가 시간 · 정체율 변화"]
    F --> G{"V2 유지 ·<br/>우회 ≤ 10분?"}
    G -- "예" --> H["⑤ 경유지 확정<br/>슬롯당 1곳 + 대안 5곳"]
    G -- "아니오" --> I["다음 후보로 교체<br/>3회 실패 시 슬롯 포기"]
    I --> E
    H --> J["⑥ LLM 한 줄 설명 (미구현)<br/>리뷰 요약 · 선정은 못 바꿈"]
...
| 슬롯 | 기준 지점 | 통과 예정 시각 조건 | 대상 |
| --- | --- | --- | --- |
| 🍚 **가는 길 식사** | 편도 60% 지점 | 식사 시간대 (11~14시 **또는** 17~21시) | 한식 식당 |
| ☕ **가는 길 카페** | 편도 80% 지점 | 카페 시간대 (10~22시) | 카페 |
| ☕ **오는 길 카페** | 목적지 | 귀가 출발 시각(T2)이 카페 시간대 | 카페 |
| 🍚 **오는 길 식사** | 목적지 | 귀가 도착 시각(T3)이 식사 시간대 | 한식 식당 |

- 시간대 판정은 **하나의 술어**를 두 식사 슬롯에 공통으로 씁니다. 가는 길이 저녁 시간대에 걸리면 가는 길 식사 슬롯도 열립니다.
- 해당 시간대를 지나지 않는 코스는 그 슬롯을 **비웁니다**. 편도 15 km 코스로 오전 9시에 출발하면 식사 슬롯은 아예 생기지 않습니다.
- 최대 슬롯은 **4개**이고, 실제로 채워지는 수는 우회 시간 합계 예산(25분)이 함께 제한합니다.
- 오는 길 슬롯은 §3.3에 따라 **통계 예측(α = 0)** 위에 세워지므로, 카드에 `예상` 배지를 붙이고 도착 후 재조회를 권합니다.
- 슬롯이 시간적으로 겹칠 때의 우선순위 규칙은 **아직 없습니다**(§9). 조건을 만족하는 슬롯이 모두 열립니다.
...
| 설명 | LLM | 리뷰를 한 줄로 요약 | 없음 (선정을 못 바꿈) · **미구현** |
| 결정 | 사용자 | 대안 5곳 중에서 교체 | 있음 (최상위) |
...
| 리뷰 수 | **15건 이상** | 경유지는 후보 밀도가 낮아 목적지 기준(30건)보다 완화 |
| 평점 | 4.0 이상 | — |
...
POI 점수 = 0.4 × 평점정규화
         + 0.3 × (1 − 우회시간 ÷ 10분)
         + 0.2 × 리뷰수정규화            리뷰수정규화 = log10(1+리뷰) ÷ log10(501)
         + 0.1 × 주차가능
         + 0.08 × 조망키워드             카페 슬롯에서만
         − 0.10 × 영업시간미확인
```
...
"분위기 좋은 카페"는 정성적이라 점수식에 직접 넣지 않습니다. 대신 카페 슬롯에서는 리뷰 텍스트에 **뷰 · 테라스 · 루프탑 · 창가 · 노을** 등 조망 키워드가 나타난 비율을 리뷰수 항목의 가산 요소로 쓰고, 실제 표현은 LLM 요약 문장에 맡기려 했으나, 현재는 카드에 `조망 키워드 있음` 배지를 붙이는 데서 멈춥니다(§9). 정성 판단을 코드가 흉내 내려 하면 §3에서 세운 재현성이 무너집니다.
...
| 편도 정체율 상승 | **2%p 이하** 이면서 절대값이 V2(8%)를 넘지 않을 것 | 다음 후보로 교체 | ❌ 미구현 (§9) |
| 교체 시도 | 슬롯당 **3회** | 해당 슬롯 포기 · 사유 표기 | ✅ |

우회 시간은 경로를 다시 탐색해 재는 것이 아니라 **직선거리 근사**로 냅니다.

```
우회 추가 시간(분) ≈ 기준 지점과 POI 사이 직선거리 ÷ 30 km/h × 60 × 2   (왕복)
```

경유지 하나마다 경로를 재탐색하면 슬롯당 3회 × 4슬롯 = 최대 12회의 추가 호출이 생겨 §10의 쿼터 상한을 깹니다. 근사를 쓰는 대신 정체율 상승 검증(V4의 2%p 항목)과 경유지 반영 연비 재산출을 포기한 상태이며, 이것이 현재 스펙과 구현이 갈라지는 가장 큰 지점입니다.

- 경유지가 확정되면 §5.2의 예상 연비를 재산출해야 합니다. 우회 구간의 도로 등급이 바뀌기 때문입니다. **현재는 재산출하지 않아** 화면의 연비·기름값이 경유지 없는 경로 기준으로 남습니다(§9).
...
| 로컬 상태 | `localStorage` — 아래 표 참조 |
| 오프라인 | 정적 SVG 경로로 **기능 축소 동작** (§7.3) |

#### `localStorage` 에 실제로 남는 것

| 키 | 내용 | 쓰는 곳 |
| --- | --- | --- |
| `sd.detour.<존>` | 존별 우회계수 학습값 (지수이동평균 0.8/0.2) | §2 ② 밴드 산출 |
| `sd.snapshot` | 직전 `courseSetId` · 출발지 · 밴드 · 후보 9개 | §2 재현성 |
| `sd.fuelPrice` | 마지막으로 고른 유종 단가 | §5.3 |
| `theme` | 다크 · 라이트 선택 | 화면 |
| `apiUsage` | 날짜별 API 호출 수 (자정 리셋) | 푸터 디버그 줄 |

차종과 실연비 이력은 아직 저장하지 않습니다. 프리셋·고속도로·체류 시간·존 선택도 세션 안에서만 유지되고 새로고침하면 기본값으로 돌아갑니다(§9).
...
| 요소 | 동작 |
| --- | --- |
| `갈 때` / `올 때 (예측)` 토글 | 귀로 탐색(§3.3 L1/L2)이 끝나면 나타나며, 같은 지도에서 두 경로를 바꿔 그림 |
| 색상 범례 | 원활 · 서행 · 정체 · `정보 없음 / 예측` 4단계 |

`정보 없음`과 `예측`을 같은 회색으로 묶은 것은 둘 다 **실시간으로 확인되지 않은 구간**이라는 점이 같기 때문입니다.
...
| 지도 SDK 로드와 API 응답 대기를 겹침 | 초기 렌더 지연 흡수. TMAP `jsv2` 는 내부에서 `document.write` 를 쓰는 부트스트랩이라 **동적 삽입이 불가**하고, 파서가 읽는 자리에 태그를 만들어야 한다 |
...
POI 검색이 1.5초를 넘기면 **기다리지 않고** 직전 `courseSetId` 스냅샷으로 후보를 채운 뒤, 새 응답이 오면 목록만 조용히 갱신하는 것이 설계입니다. **현재는 미구현**이며, 스냅샷은 저장만 되고 읽히지 않습니다 — POI 응답을 끝까지 기다립니다(§9).
...
| 영역 | 내용 |
| --- | --- |
| 컨트롤 | 선호 거리 · 고속도로 · 체류 시간 · 경유지 토글 · 출발 존 · **유종** · 지정 모드 입력(출발지/목적지) · `다시 탐색` |
| 지도 | 경로 폴리라인, 갈 때/올 때 토글, 색상 범례, SVG 폴백 |
| 요약 카드 | 편도 km · 왕복 주행 시간 · 편도 정체율 · 예상 연비 · **예상 기름값** |
| 후보 목록 | 랭킹순 9개. 미탐색 후보는 추정 표기, 누르면 그 후보로 교체 |
| 연비표 | 구간별 거리 · 비중 · 구간 연비 (왕복 기준) + 예상 주유량과 기름값 |
| 경유지 카드 | 슬롯별 확정 1곳 + 대안, 실패 시 사유 표기 |
| 푸터 디버그 | `courseSetId` · API 호출(오늘/세션) · 귀로 통계 단계(L1/L2/L3) · 첫 렌더 ms |

- 다크 · 라이트 **테마 토글**이 있고 선택은 `localStorage`에 남습니다. 테마를 바꾸면 Leaflet 타일도 다시 그립니다.
- 판정에 영향을 준 사건(밴드 확장, 실시간 교통 없음, 목 데이터 사용, V2 미달)은 **배너**로 화면에 남깁니다. 조용히 넘어가면 사용자가 추정치를 실측으로 오해합니다.
...
현재 구현으로 **측정 가능한 항목은 V1 · V1' · V2 · V2' · V5 · V5' · V6'** 이다. V3(실연비 입력 없음), V4의 정체율 상승 항목(검사 미구현), V5''(스냅샷 리플레이 미구현), V6(실측 입력 없음)은 아직 잴 수 없다. 자세한 경계는 §9의 구현 현황 표에 있다.
...
[`server.py`](server.py) 는 `SimpleHTTPRequestHandler` 에 엔드포인트 하나를 얹은 것이다.

| 경로 | 동작 |
| --- | --- |
| `/api/fuel` | 전국 주유소 평균가를 조회해 `{gasoline, premium, diesel}` JSON 반환 (§5.3) |
| 그 밖 | 정적 파일 서빙 |

유가 조회는 브라우저에서 직접 하면 CORS에 막히므로 서버를 한 단계 둔다. `python3 -m http.server` 로 열어도 페이지는 동작하지만, `/api/fuel` 이 404가 나서 유가는 §5.3의 초기 단가로 고정된다.

에디터에서는 [`.claude/launch.json`](.claude/launch.json) 의 `smooth-drive` 구성이 같은 명령을 실행한다.
...
새로고침하면 전환된다. 화면 **우측 하단**의 흐린 글씨에 어느 키가 잡혔는지 표시된다 — `config.local.js 없음 · 목 데이터` / `config.local.js · TMAP + Kakao 사용 중` 처럼.
...
문서가 앞서고 코드가 뒤따르는 구간이 있다. 아래 표가 그 경계다.
...
| 나머지 6개 후보 | 사용자가 고른 시점에 탐색 | 첫 렌더 뒤 백그라운드 순차 탐색 (§2 ⑥) | 사용자가 목록을 훑는 동안 값이 채워지는 편이 낫다. 5초 예산 밖이라 V1은 그대로 |
| 경유지 기본값 | 꺼짐 | **켜짐** (§6) | 비동기로 분리해 두면 첫 렌더를 늦추지 않는다 |
| 경유지 리뷰 수 하한 | 30건 | **15건** (§6.2) | 목적지보다 POI 밀도가 낮아 30건으로는 슬롯이 자주 빈다 |
| 우회 시간 측정 | 경로 재탐색 후 비교 | 직선거리 근사 (§6.3) | 슬롯당 3회 × 4슬롯의 추가 호출이 쿼터를 깬다 |

#### 아직 구현하지 않은 것

| 항목 | 해당 절 | 현재 상태 | 영향 |
| --- | --- | --- | --- |
| LLM 설명 계층 | §3 · §6.2 | 규칙 기반 템플릿 문장으로 대체 | 추천 이유·리뷰 요약이 정형 문구에 머문다 |
| 경유지 정체율 상승 검증 | §6.3 · V4 | 예산 상수는 있으나 검사 코드가 없다 | **V4의 2%p 항목을 측정할 수 없다** |
| 경유지 반영 연비 재산출 | §6.3 | 경유지 없는 경로 기준값이 남는다 | 경유지를 켜면 연비·기름값이 실제와 어긋난다 |
| L3 실측 보정 | §3.3 | 표본·계수를 읽기만 하고 쓰는 경로가 없다 | 항상 `L3 보정 없음 · 표본 0/30` |
| 실연비 입력과 V3 보정 루프 | §5.2 · V3 | 공인연비 고정값만 사용 | **V3를 측정할 수 없다** |
| 스냅샷 폴백 · 리플레이 | §2 · §7.4 · V5'' | 저장만 하고 읽지 않는다 | POI 지연 시 그대로 대기, 회귀 테스트 불가 |
| 슬롯 시간 충돌 해소 | §6.1 | 조건을 만족하는 슬롯이 모두 열린다 | 같은 시간대에 두 곳이 배정될 수 있다 |
| 설정 영속화 | §7.1 | 프리셋·고속도로·체류·존은 세션 한정 | 새로고침하면 기본값으로 돌아간다 |

#### 알려진 결함

| 위치 | 내용 |
| --- | --- |
| `index.html` 목 데이터 배너 | "파일 상단 `CONFIG`에 키를 넣으라"고 안내하지만 실제 키 위치는 `config.local.js` 다 (§9). 문구가 옛 구조를 가리킨다 |
| `server.py` 유가 파서 | 검색 결과 HTML의 표 형식에 정규식으로 의존한다. 형식이 바뀌면 조용히 `{error}` 를 반환하고 초기 단가로 폴백된다 |
...
| 첫 방문에 POI API가 죽으면 추천 모드가 아예 동작하지 않음 | 고정 풀을 없앤 직접적 대가. 스냅샷 폴백이 미구현(§9)이라 현재는 목 데이터 또는 지정 모드 안내로만 넘어간다 |
...
| 귀로 통계 예측의 정확도 | §3.3의 L1 → L2 → L3 폴백으로 출처를 확정. L1(타임머신)이 붙으면 링크 단위, 실패 시 등급 단위 근사로 내려감. L3은 아직 동작하지 않음(§9) |
...
| 유가 출처의 안정성 | 검색 결과 HTML 파싱이라 형식 변경에 취약. 공공 오피넷 API로 옮기는 편이 안전 (§9) |
| 차종별 연비 편차 | 최초 1회 차종 등록 후 실연비로 개인화 |
| 지도 API 요금·쿼터 | 첫 렌더는 경로 3회로 제한하되, 백그라운드 탐색까지 합하면 세션당 최대 9회(§2 ⑥). 일일 호출 수를 `localStorage`에 세어 푸터에 표시 |
...
| 경유지 사용 시 API 호출 증가 | 경로 재탐색을 하지 않고 직선거리로 근사해 추가 호출은 슬롯당 POI 조회 1회뿐 (§6.3) |

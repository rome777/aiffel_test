...
9개를 만들면서 경로 탐색은 3개만 하는 이유는, 9개를 모두 탐색하면 §8 V1의 5초 예산과 §9의 쿼터 상한을 동시에 깨기 때문입니다. 나머지 6개는 목록으로만 보여 주고 사용자가 고른 시점에 탐색합니다.
...
| 순위 | 소스 | 정밀도 | 추가 호출 |
| --- | --- | --- | --- |
| **L1** | 경로안내 API의 **미래 출발 시각 탐색** (`departureTime`) | 링크 단위 | **1회** |
...
| 후보 | 파라미터 | 지원 |
| --- | --- | --- |
| TMAP 경로안내 | `departureTime` (RFC3339) | 상품별로 다름 — 키 발급 시 확인 필요 |
| Naver Directions 5 | `departure` | 옵션 제공 |
| Kakao Navi | — | 미지원 |

T2 시각을 넣어 귀로를 한 번 탐색하면 링크 단위 예측이 그대로 옵니다. 지원하지 않는 SDK이거나 호출이 실패하면 L2로 내려갑니다.
...
사용자가 실제로 지난 구간의 소요 시간을 누적해 `보정 계수 = 실측 속도 ÷ 예측 속도`를 만들고 L1 · L2 결과에 곱합니다. **표본 30건 미만 구간은 보정하지 않습니다**(§9). 주말에만 쓰는 개인 기록으로는 표본이 더디게 쌓이므로, L3은 단독 소스가 아니라 위 두 단계를 덧칠하는 층입니다.
...
| 리뷰 수 | **30건 이상** | 표본 부족을 배제하는 §9의 기준과 동일 축 |
| 평점 | 4.0 이상 | — |
...
| 귀로 통계 예측의 정확도 | §3.3의 L1 → L2 → L3 폴백으로 출처를 확정. L1 미지원 SDK에서는 등급 단위 근사에 머무름 |

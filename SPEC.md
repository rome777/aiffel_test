이 문서는 실행·배포 방법, 판단 규칙·산식·아키텍처의 근거를 담은 상세 스펙입니다.
...
`http://localhost:4173`으로 엽니다. 빌드 없음, `index.html` 단일 파일입니다(`file://`은 CORS로 막혀 안 됩니다). 키를 안 넣으면 목(mock) 데이터로 동작하고, 판정 로직(후보 생성·랭킹·연비·경유지)은 전부 실제로 돌되 외부 응답만 합성으로 대체됩니다.
